...
import { getCDTs, deleteCDT } from "../api";
import LogoutButton from "../components/LogoutButton";

const NEOCDTList: React.FC = () => {
...
const handleEliminar = async (id: string) => {
  const confirmar = window.confirm("¿Seguro que deseas eliminar este CDT?");
  if (!confirmar) return;
  ...
  try {
    const res = await deleteCDT(id, token); // deleteCDT lanza error si res.ok === false
    alert(res.message || "CDT eliminado correctamente.");
    // Refrescar lista
    const nuevos = await getCDTs(token);
    setCdts(nuevos);
  } catch (err: any) {
    // Mostrar mensaje de error real del backend
    if (err.message) {
      alert(`No se pudo eliminar el CDT: ${err.message}`);
    } else {
      alert("Ocurrió un error al eliminar el CDT.");
    }
    console.error(err);
  }
};
  ...
  const handleEditar = (id: string) => {
    window.location.href = `/cdts/edit/${id}`;
  };
        ...
        <div className="wrap nav">
          <a href="/" className="brand">
            ...
            <span className="brand__text">
              <span className="brand__neo">NEO</span>CDT
            </span>
          </a>
          ...
          <nav className="nav__links">
            ...
            <a href="/tasas" className="navlink">
              Simulador
            </a>
          </nav>
      ...
      <main className="wrap main">
        <section className="card">
          <div className="card__head">
            ...
            <a href="/cdts/new" className="btn btn--success">
              Crear nuevo CDT
            </a>
          </div>
                ...
                {cdts.map((cdt: any) => (
                  <li key={cdt._id} className="cdt">
                    <div className="cdt__row">
                      ...
                      <strong className={`status ${statusClass(cdt.estado)}`}>
                        {cdt.estado}
                      </strong>
                    </div>
                    ...
                    {/* Botones CRUD */}
                    <div className="cdt__actions">
                      <button
                        className="btn btn--ghost"
                        onClick={() => handleEditar(cdt._id)}
                      >
                        Editar
                      </button>
                      ...
                      {cdt.estado === 'Pendiente' && (
                        <button
                          ...
                          onClick={() => handleEliminar(cdt._id)}
                        >
                          Eliminar
                        </button>
                      )}

                      {cdt.estado?.toLowerCase() === "activo" && (
...
function statusClass(estado?: string) {
  ...
  if (s === "active") return "status--neutral";
  if (s === "aprobado" || s === "activo") return "status--ok";
  if (s === "pendiente por aprobación") return "status--neutral";
  if (s === "rechazado" || s === "cancelado" || s === "vencido")
    return "status--bad";
  return "status--neutral";
}

/* --- CSS (sin Tailwind) --- */
export default NEOCDTList;
