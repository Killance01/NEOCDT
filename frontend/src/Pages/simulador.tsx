...
export default function Tasas() {
  ...
  const res = useMemo(() => {
    ...
    const plazoNum = Number(plazoDias) || 0;
    const tasaNum = Number(tasaEA) || 0;
    const retPct = Number(retencionPct) || 0;

    const ea = tasaNum / 100;
    const beneficioGMF = gmfActivo ? montoNum * 0.004 : 0;
    const totalInicial = montoNum + beneficioGMF;
    const rDia = Math.pow(1 + ea, 1 / baseDias) - 1;
    const rendimiento = totalInicial * (Math.pow(1 + rDia, plazoNum) - 1);
    const retencion = rendimiento * (retPct / 100);
    ...
    <div>
      ...
      <div className="wrap">
          ...
          <div className="formGrid">
            ...
            <label className="field checkbox">
              <input
                type="checkbox"
                checked={gmfActivo}
                onChange={(e) => setGmfActivo(e.target.checked)}
              />
            ...
            <label className="field">
              ...
              <select
                value={baseDias}
                onChange={(e) => setBaseDias(Number(e.target.value) as 365 | 360)}
              >
        ...
        <section className="card">
          ...
          <div className="rows">
            <Row label="Monto Inicial" value={fmt(Number(monto))} />
            ...
            <Row label="Tasa de Interés EA" value={`${Number(tasaEA).toFixed(2)} %`} />
            <Row
              label={`Retención en la fuente ${retencionPct} %`}
              value={fmt(res.retencion)}
              info
              divider
            />
            <Row
              ...
              label={
                <>
                  Rentabilidad de tu CDT al <br /> vencimiento
                </>
              }
              value={fmt(res.totalVenc)}
            />
          </div>
          <div className="ctaBox">
            <button className="btnPrimary" onClick={() => navigate("/cdts/new")}>
              Quiero mi CDT
            </button>
          </div>
        </section>
...
function Row({
  ...
  return (
    <div
      className={`row ${divider ? "row--divider" : ""} ${
        highlight ? "row--hl" : ""
      }`}
    >
      <div className="label">
