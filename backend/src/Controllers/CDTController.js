...
// Actualizar CDT
const actualizarCDT = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (cdt.usuarioId.toString() !== usuarioId) {
      return res.status(403).json({ error: 'No puedes editar este CDT' });
    }

    if (cdt.estado !== 'Pendiente') {
      return res.status(400).json({ error: 'Solo se pueden editar CDTs pendientes' });
    }

    cdt.monto = monto;
    cdt.plazo = plazo;
    cdt.fechaVencimiento = new Date(Date.now() + Number(plazo) * 24 * 60 * 60 * 1000);

    await cdt.save();
    res.json({ message: 'CDT actualizado correctamente', cdt });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
